...
from utils.plots import Annotator, colors, save_one_box, save_one_frame
...
def run(
        ...
        weights='/usr/src/app/runs/train/704-64-150-n/weights/best.pt',
        # file/dir/URL/glob/screen/0(webcam)
        source='rtsp://192.168.0.101:8554/video',
        ...
        name=str(datetime.now()).replace(" ", "").replace(
            "-", "").replace(":", "").replace(".", ""),  # save results to project/name
    ...
    # First frame will always save crops
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='/usr/src/app/runs/train/704-64-150-n/weights/best.pt', help='model path or triton URL')
    parser.add_argument('--source', type=str, default='rtsp://192.168.0.101:8554/video',
                        help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT /
                        'datasets/BRPlates.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+',
                        type=int, default=[704], help='inference size h,w')
    ...
    parser.add_argument('--name', default=str(datetime.now()).replace(" ", "").replace(
        "-", "").replace(":", "").replace(".", ""), help='save results to project/name')
    ...
    parser.add_argument('--save-frame', action='store_true',
                        help='save frame to project/name/exp*/frames/frame*.png')
    parser.add_argument('--save-detected-frame', action='store_true',
                        help='save frame with detection to project/name/exp*/frames/frame*.png')
    parser.add_argument('--save-each-n-frames', type=int, default=1,
                        help='frequency for saving frames when setting save-frame or save-detected-frame (trying to decrease img saving delay.)')
