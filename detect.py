...
from ultralytics.utils.plotting import Annotator, colors, save_one_box, save_one_frame
...
def run(
    weights=ROOT / "/usr/src/app/runs/train/320--1-n6-14/weights/best.pt",  # model path or triton URL
    ...
    .replace("-", "")
    .replace(":", "")
    .replace(".", ""),  # save results to project/name
    ...
    save_detected_frame=False,  # save frame from video when category detected
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop and (frames_since_last_saved == save_each_n_frames):
                        save_one_box(
                            xyxy, imc, file=save_dir / "crops" / names[c] / f"{id}_{det_in_frame}.jpg", BGR=True
                        )
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=ROOT / "/usr/src/app/runs/train/704-64-150-n/weights/best.pt",
        help="model path or triton URL",
    )
    parser.add_argument(
        "--source",
        type=str,
        default=ROOT / "rtsp://192.168.0.101:8554/video",
